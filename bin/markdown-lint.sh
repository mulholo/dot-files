...
do
    if [[ $PAGE == *.page ]]; then
        ...
        fgp -e 'http://dl.dropbox' -e 'http://news.ycombinator.com' -e 'http://github.com' \
            ...
            -e 'webarchive.org.uk/wayback/' -e 'plus.google.com/' -e 'www.deepdotweb.com' -e 'raikoth.net' \
            -e 'drive.google.com/file' -e 'ssrn.com' -e 'ardenm.us' -e 'gnxp.nofe.me' -- "$PAGE";
        ...
        export -f checkPDF
        # examples: no/no/yes
        ## checkPDF 'http://www.nytimes.com/2009/11/15/business/economy/15view.html '
        ## checkPDF 'http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.208.2314&rep=rep1&type=pdf'
        ## checkPDF 'https://files.osf.io/v1/resources/np2jd/providers/osfstorage/59614dec594d9002288271b6?action=download&version=1&direct'
        link-extractor.hs "$PAGE" | egrep "^http" | sort -u | shuf | parallel --progress -n 1 checkPDF
        ...
        linkchecker --no-status --check-extern --threads=3 --timeout=20 -r1 --ignore='file://.*' "$HTML"
    fi
    if [[ $PAGE == *.sh ]]; then
        shellcheck "$PAGE"
    fi
